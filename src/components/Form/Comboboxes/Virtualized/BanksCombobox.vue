...
const props = defineProps({
  ...
  emptyMessage: {
    type: String,
    default: 'Nenhum banco encontrado.'
  },
  label: {
    type: String,
    default: 'Banco'
...
const fetchSelectedLabel = async (bankId: number) => {
  try {
    const bank = await api.get(bankId)
    selectedLabelValue.value = `${bank.code} - ${bank.name}`
  } catch (error) {
    console.error('Error fetching selected bank:', error)
  }
}
